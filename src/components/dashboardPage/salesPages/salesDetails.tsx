...
import { IDocument } from "@/models/construction";
...
export default function SalesDetails() {
    ...
    const { data: document, isLoading: loadingdata } = useFetchData<Array<IDocument>>(`/documents/type/RESERVATION/id/${data?.id}`, ["documents", data?.id + ""]);
                    ...
                    <div className=" w-full flex gap-4 " >
                        <div className=" w-full flex flex-col gap-4 " >
                            ...
                            <div className=" w-full p-4 rounded-2xl flex flex-col gap-3 border " >
                                ...
                                <LoadingAnimation loading={loading} length={payment?.length} >
                                    <Table>
                                        <TableHeader>
                                            <TableRow>
                                    ...
                                    <Table>
                                        ...
                                        <TableBody>
                                            {document?.map((item, index) => {
                                                return (
                                                    <TableRow className={` h-[72px] px-3 ${(index % 2 === 0) ? "bg-gray25" : ""} `} key={index}>
                                                        <TableCell className="">{item?.documentType}</TableCell>
                                                        <TableCell>
