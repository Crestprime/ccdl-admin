...
export default function PropertyListing({data, type}: {data: Array<CreateBuildingListing & CreateLandListing & any>, type: "buildings" | "lands"}) {
    ...
    return (
        <div className=" w-full grid grid-cols-4 gap-6 pb-6" >
            {data?.map((item, index) => (
                <div onClick={()=> navigate(`/dashboard/property/listings/details?id=${item?.id}&type=${type}`)} role="button" key={index} className=" w-full flex flex-col gap-2 pb-4 rounded-xl" >
